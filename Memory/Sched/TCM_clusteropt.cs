...
{
    public class SchedTCMClusterOpt : Scheduler
    {
        //rank
        public int[] rank;
	...
	public int voting_for_factor_up;
	public int voting_for_factor_down;
	public double[] pre_progress;

	public bool log_already_output;

	public double accelerate_probability_nonint;
	public double accelerate_probability_int;

	public int quantum_cycles_for_probability;
	public int quantum_cycles_left_for_probability;

        public SchedTCMClusterOpt(SchedBuf[] buf, DRAM mem, Channel chan) : base(buf,mem,chan)
        {
...
//		Console.Write(",{0}", hwa_prior[Config.HWANum-i-1]);
	...
	override public int getPriority( int id )
	{
	    ...
	    // 4. memory intensive CPU Cluster (When accelerated)
	    // 5. Low Priority in long deadline group: HWANum+1-2*HWANum
	    // 6. non-memory intensive CPU Cluster (When not accelerated): 2*HWANum+1
	    // 7. memory intensive CPU Cluster(When not accelerated)
...
//		   Simulator.QoSCtrl.schedule_cluster_check(1, id, chan.mem_id, chan.id ) && ( icluster_size > 0 ) && ( pre_progress[id] < progress ))
		   Simulator.QoSCtrl.schedule_cluster_check(1, id, chan.mem_id, chan.id ) && ( quantum_cnt > 0 ) && ( pre_progress[id] < progress ))
		{

		    if(( progress - pre_progress[id] ) > ( (double)Config.sched.qosEpoch / (double)Simulator.network.nodes[id].cpu.deadLine ) )
		    {
			voting_for_factor_up++;
			Console.WriteLine("ClusterAdjust vote4up up:{0} down:{1}", voting_for_factor_up, voting_for_factor_down );
		    }
		    else if( ( progress - pre_progress[id] ) < ( (double)Config.sched.qosEpoch / (double)Simulator.network.nodes[id].cpu.deadLine ) )
		    {
			voting_for_factor_down++;
			Console.WriteLine("ClusterAdjust vote4down up:{0} down:{1}", voting_for_factor_up, voting_for_factor_down );
		    }
...
//		Console.WriteLine("Pid:{0}, RBL:{1}, dramreq:{2}, delta_cache_miss:{3}", p, rbl[p],Simulator.stats.dramreqs_persrc[p].Count,delta_cache_miss);
	...
	{
	    if( voting_for_factor_up > voting_for_factor_down )
	    {
		if( Config.sched.is_clustering_and_probability )
		{
		    if( accelerate_probability_nonint + 5 <= 100 )
		    {	
			accelerate_probability_nonint += 5;
			Console.WriteLine("ClusterAdjustProbNonInt {0}->{1}", accelerate_probability_nonint-5, accelerate_probability_nonint);
		    }
		    else if( Config.sched.is_clustering_and_probability_dual && ( accelerate_probability_int + 5 <= 100 ))
		    {
			accelerate_probability_int += 5;
			Console.WriteLine("ClusterAdjustProbInt {0}->{1}", accelerate_probability_int-5, accelerate_probability_int);
		    }
		    else if( Config.sched.is_clustering_th_and_prob )
		    {
			if( cluster_factor < 0.7 )
			{
			    cluster_factor += 0.05;
			    Console.WriteLine("ClusterAdjust {0}->{1}", cluster_factor-0.05, cluster_factor );
			}
		    }
		}
		else
	    ...
	    else if( voting_for_factor_up < voting_for_factor_down )
	    {
		if( Config.sched.is_clustering_and_probability )
		{
		    if( Config.sched.is_clustering_and_probability_dual && ( accelerate_probability_int - 5 >= 0 ))
		    {
			accelerate_probability_int -= 5;
			Console.WriteLine("ClusterAdjustProbInt {0}->{1}", accelerate_probability_int+5, accelerate_probability_int);
		    }
		    else if( accelerate_probability_nonint - 5 >= 0 )
		    {	
			accelerate_probability_nonint -= 5;
			Console.WriteLine("ClusterAdjustProbNonInt {0}->{1}", accelerate_probability_nonint+5, accelerate_probability_nonint);
		    }
	    ...
	    voting_for_factor_up = 0;
	    voting_for_factor_down = 0;
	}
