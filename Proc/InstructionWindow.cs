...
namespace ICSimulator {
    ...
    public class InstructionWindow {
        ...
        public int next, oldest;

        public ulong[] issueT, headT;

	/* HWA CODE */
	public bool[] dram_req;
	    ...
	    /* HWA CODE */
	    dram_req = new bool[Config.proc.instWindowSize];
        ...
        virtual public void fetch(Request r, ulong address, bool isWrite, bool isReady) {
...
//		if (!isReady)
//		    Console.WriteLine("pr{0}: outstanding reqs now at {1}, slot:{2},addr:{3:X}", m_cpu.ID, outstandingReqs, next,address);
        ...
        virtual public int retire(int n) {
            int i = 0;
		...
		if( dram_req[oldest] )
		{
		    Simulator.stats.slackPerSrc[m_cpu.ID].Add(deadline);
		    Simulator.stats.slackReqCntPerSrc[m_cpu.ID].Add(1);
		}
		/* HWA CODE END */
                oldest++;
        ...
        virtual public bool contains(ulong address, bool write) {
            int i = oldest;
            while (i != next) {
        ...
        virtual public void setReady(ulong address, bool write) {
...
//                    Console.WriteLine("pr{0}: set slot {1} ready, now {2} outstanding", m_cpu.ID, i, outstandingReqs);
	...
	/* HWA Code */
	public void setDramReq(int windowID)
	{
	    dram_req[windowID] = true;
	}
    }
}
