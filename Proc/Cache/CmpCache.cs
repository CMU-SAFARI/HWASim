...
namespace ICSimulator
{
    ...
    public class CmpCache_Pkt
    {
        ...
        public bool is_GPU; // is this a pkt generated from the GPU
...
//                    m_sh = new Sets<CmpCache_State>(m_blkshift, 1 << Config.sh_cache_assoc, (1 << (Config.sh_cache_size - Config.cache_block - Config.sh_cache_assoc)) * m_N);
		{
			...
			m_sh = new Sets<CmpCache_State>(m_blkshift, 1 << Config.sh_cache_assoc, (1 << (Config.sh_cache_size - Config.cache_block - Config.sh_cache_assoc)) * Config.NumberOfNodeHavingShCache );
		    else
			m_sh = new Sets<CmpCache_State>(m_blkshift, 1 << Config.sh_cache_assoc, (1 << (Config.sh_cache_size - Config.cache_block - Config.sh_cache_assoc)) * m_N);
		}
            }
...
//                if(!Simulator.network.nodes[node].cpu.is_GPU())
        ...
        void do_evict(CmpCache_Txn txn, CmpCache_Pkt init_dep, int node, ulong evict_addr, out bool wb)
        {
            ...
            {
                CmpCache_Pkt wb_pkt = add_data_pkt(txn, node, sh_slice, false, false);
            ...
            {
                CmpCache_Pkt release_pkt = add_ctl_pkt(txn, node, sh_slice, false, false);
        ...
        CmpCache_Pkt do_inval(CmpCache_Txn txn, CmpCache_State state, CmpCache_Pkt init_dep, int node, ulong addr, int c2c)
        {
            ...
            for (int i = 0; i < m_N; i++)
                ...
                {
                    CmpCache_Pkt invl_pkt = add_ctl_pkt(txn, sh_slice, i, false, false);
                    invl_pkt.delay = m_shdelay;

                    CmpCache_Pkt invl_resp =
                        (c2c == i) ?
                        add_data_pkt(txn, i, node, false, false) :
                        add_ctl_pkt(txn, i, node, false, false);
        ...
        CmpCache_Pkt _add_pkt(CmpCache_Txn txn, int from, int to, bool data, bool send, bool done, bool is_GPU)
        {

            //Console.WriteLine("Adding a packet, isGPU = {0}",is_GPU);
        ...
        CmpCache_Pkt add_ctl_pkt(CmpCache_Txn txn, int from, int to, bool done, bool is_GPU)
        {
            //Console.WriteLine("Adding a control packet, isGPU = {0}",is_GPU);
            return _add_pkt(txn, from, to, false, true, done, is_GPU);
        }

        CmpCache_Pkt add_data_pkt(CmpCache_Txn txn, int from, int to, bool done, bool is_GPU)
        {
            //Console.WriteLine("Adding a data packet, isGPU = {0}",is_GPU);
            return _add_pkt(txn, from, to, true, true, done, is_GPU);
        }

        CmpCache_Pkt add_joinpt(CmpCache_Txn txn, bool done)
        {
            return _add_pkt(txn, 0, 0, false, false, done, false);
        }
        ...
        void send_pkt(CmpCache_Txn txn, CmpCache_Pkt pkt)
        {
            ...
            else if (pkt.send)
            {
                send_noc(txn.node, pkt.from, pkt.to, pkt.flits,
                        ...
                        }, pkt.off_crit, pkt.vc_class, pkt.is_GPU);
            }
            else if (pkt.mem)
            {
        ...
        void send_noc(int reqNode, int from, int to, int flits, Simulator.Ready cb, bool off_crit, int vc, bool is_GPU)
        {
            int cl = off_crit ? 2 : // packet class (used for split queues): 0 = ctl, 1 = data, 2 = off-crit (writebacks)
                (flits > 1 ? 1 : 0);

            CachePacket p = new CachePacket(reqNode, from, to, flits, cl, vc, cb, is_GPU);
