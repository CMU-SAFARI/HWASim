...
namespace ICSimulator
{
    ...
    public class MultiQThrottlePktPool : IPrioPktPool
    {
        ...
        public Packet next()
        {
            ...
            // queue_next == 0 is a injection queue for control packet e.g. data request. 
            if (queues[queue_next].Count > 0 &&
                    (queue_next != 0 || Simulator.controller.tryInject(node_id)))
            {
                Packet p = queues[queue_next].Dequeue();
            ...
            else if (queues[queue_next].Count > 0 && queue_next == 0)
            {
                Simulator.stats.starve_flit_throttle_bysrc[queues[queue_next].Peek().src.ID].Add();
                //Console.WriteLine("q throttled");
                return null;
            }
            return null;
        }
    ...
    public class ReallyNiftyPrioritizingPacketPool : IPrioPktPool
    {
        ...
        void advanceRR()
        {
            ...
            //while (tries-- > 0 && heaps[heap_next].Count == 0 );
