...
namespace ICSimulator
{
    ...
    public class Controller_STC : Controller_ClassicBLESS
    {
        ...
        {
            // TODO: remove me
            //throw new Exception("STC");
            // rule 1: older batch (lower batchID) is greater
            int cmp1 = 0;
            // Need to take care of wrap around
            int curBID  = (int)(Simulator.CurrentRound / Config.STC_batchPeriod) % (int)Config.STC_batchCount;
            int f1RelativeBPrio = (curBID - (int)f1.packet.batchID) % (int)Config.STC_batchCount;
            int f2RelativeBPrio = (curBID - (int)f2.packet.batchID) % (int)Config.STC_batchCount;
            // < 0 iff a has higher priority
            if (f1RelativeBPrio > f2RelativeBPrio)
                cmp1 = -1;
            else if (f1RelativeBPrio < f2RelativeBPrio) 
                cmp1 = 1;
            else
                cmp1 = 0;
